...
class Paginator:
    ...
    def total_pages(self) -> int:
        ...
        # BUG 3: This creates an extra empty page when evenly divisible
        total = len(self.items) // self.per_page
        if len(self.items) % self.per_page != 0:
            total += 1
        else:
            total += 1  # BUG! Should not add 1 when evenly divisible
...
class SiteBuilder:
    ...
    def parse_content(self):
        ...
        self.parsed_content = self.parser.parse_directory(
            ...
            include_drafts=self.config.build_drafts
        )
